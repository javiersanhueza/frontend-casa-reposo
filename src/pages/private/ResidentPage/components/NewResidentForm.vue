<template>
  <q-form @submit="onSubmit">
    ...
    <q-card-section class="q-gutter-y-sm" style="margin-top: -20px">
      ...
      <div class="row">
        <div :class="['col-sm-6 col-12', screen.gt.sm ? 'q-pr-sm' : '']">
          <q-input
            ...
            @update:model-value="resident.secondaryGuardian.rut = formatRut($event)"
            dense
            outlined
          />
        </div>
    ...
    <q-card-section>
      <div class="row" v-if="screen.gt.sm">
        <div class="col-sm-6 col-12 q-pr-sm">
          <q-btn class="full-width" outline label="Volver" color="white" text-color="primary" @click="router.push({ name: 'ResidentPage' })" />
        </div>
        <div class="col-sm-6 col-12">
          <q-btn class="full-width" unelevated :label="isEdit ? 'Editar': 'Crear'" color="primary" text-color="white" type="submit" />
        </div>
      </div>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const onSubmit = async () => {
      const response = props.isEdit ? await residentStore.editResident(Number(idResident), resident.value) : await residentStore.createResident(resident.value)

      if (!!response?.resident) {
        Notify.create({
          type: 'positive',
          message: `Residente ${props.isEdit ? 'actualizado' : 'creado'} con éxito`,
          position: 'top',
          timeout: 3000
        });
    ...
    const customFilterProfession = (inputValue: string, doneFn: (callbackFn: () => void, afterFn?: (ref: QSelect) => void) => void) => {
    ...
    const customFilterReligion = (inputValue: string, doneFn: (callbackFn: () => void, afterFn?: (ref: QSelect) => void) => void) => {
